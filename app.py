...
def calc_breakeven_months(
    ...
    deposit_diff = deposit_a - deposit_b
    monthly_saving = total_b - total_a
...
MAX_CHECKLIST_SCORE = len(CHECKLIST_ITEMS) * 2
    ...
    risk_bar = (
        ...
        .encode(
            ...
            color=alt.Color(
                ...
                scale=alt.Scale(
                    domain=[0, 50, 70, 100],
                    range=["#1b9e77", "#d4b84a", "#d95f02", "#cc0000"],
                ),
                legend=None,
            ),
            tooltip=[
...
if not missing.empty:
    st.info(
        f"추정시세 미입력 매물 {len(missing)}개: "
        f"{', '.join(missing['매물명'].tolist())} — "
        f"'직접 편집' 모드에서 추정시세를 입력하면 위험도를 볼 수 있습니다."
    )
...
if len(listing_names) >= 2:
    ...
    if be_a != be_b:
        ...
        breakeven = calc_breakeven_months(
            deposit_a=row_a["보증금(만원)"],
            rent_a=row_a["월세(만원)"],
            maintenance_a=row_a["관리비(만원)"],
            deposit_b=row_b["보증금(만원)"],
            rent_b=row_b["월세(만원)"],
            maintenance_b=row_b["관리비(만원)"],
            annual_invest_return=annual_return,
            loan_ratio=loan_ratio,
            annual_loan_rate=loan_rate,
        )
        ...
        if breakeven is not None:
            st.success(
                f"**{be_a}**는 보증금이 {deposit_diff:,.0f}만원 더 많지만 "
                f"월비용이 {monthly_b - monthly_a:.1f}만원 적습니다. "
                f"**{breakeven}개월({breakeven // 12}년 {breakeven % 12}개월)** 거주하면 "
                f"총비용 기준으로 {be_a}가 더 유리해집니다."
            )
            ...
            be_chart = (
                ...
                .encode(
                    ...
                    tooltip=[
                        "매물",
                        "개월",
                        alt.Tooltip("누적비용(만원):Q", format=",.0f"),
                    ],
                )
            ...
            st.warning(
                f"**{be_a}**는 보증금도 더 많고 월비용({monthly_a:.1f}만원)도 "
                f"**{be_b}**({monthly_b:.1f}만원)보다 높습니다. "
    ...
    if not summary_df.empty:
        ...
        bubble = (
            ...
            .encode(
                ...
                tooltip=[
                    "매물명",
                    alt.Tooltip("월환산총비용(만원):Q", format=",.1f"),
                    "실거주점수",
                ],
            )
            .properties(height=300)
